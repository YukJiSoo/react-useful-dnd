...
function useDroppable({ id = uuid(), items }) {
	...
	const onDragLeave = event => {
		...
		if (
			...
			exitFlag = false;
		// 2) target element is droppable element & mouse over element is parent of target element
		if (
			eventTargetClassList.contains(DROP_ZONE_CLASS_NAME) &&
			elementWhichMouseOver.contains(event.target)
		)
			exitFlag = false;
		// 3) target element is draggable element and isn't parent of mouse over element
